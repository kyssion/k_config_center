...
| 关系 | 说明 |
|------|------|
| config_center_environment.namespace_id → config_center_namespace.id | 环境隶属命名空间，`UNIQUE(namespace_id, environment_key)` 保证命名空间内环境标识唯一 |
| config_center_configuration_group.environment_id → config_center_environment.id | 配置组隶属环境，`UNIQUE(environment_id, group_key)` 保证环境内组标识唯一 |
...
- `config_center_configuration.deleted_at`：软删除标记，NULL 表示有效。
...
| namespace_key | VARCHAR(128) | 命名空间业务标识，全局唯一，默认 `'public'` |
...
| environment_key | VARCHAR(64) | 环境业务标识（dev/test/staging/prod），`UNIQUE(namespace_id, environment_key)` |
...
| group_key | VARCHAR(128) | 配置组业务标识（编程标识符），`UNIQUE(environment_id, group_key)` |
...
索引：`index_group_namespace_environment(namespace_id, environment_id)`。
...
索引：`index_version_configuration(configuration_id, version_number DESC)`。快照写入后不再修改。
...
- **软删除**：置 `deleted_at`，部分唯一索引 `unique_configuration_key` 自动放行同 key 重建；版本与日志保留可审计。
...
3. **部分唯一索引 + 软删除**
   若用普通唯一约束 `UNIQUE(group_id, configuration_key)`，软删除后的旧记录会永久占用 key，导致同名配置无法重建。部分唯一索引 `WHERE deleted_at IS NULL` 只约束"活着"的记录：删除后同 key 可立即重建，而历史（已删）记录仍保留用于审计与版本追溯。
